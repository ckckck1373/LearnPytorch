...
def get_kernel():
    ...
    #mid_x = gaussian_N/2.0 - 1.5
    #mid_y = gaussian_N/2.0 - 1.5
    ...
    # (i,j) - (c-1.5,c-1.5) === (i+1.5,j+1.5) - (c,c)
    D_x = x + 1.5
    D_y = y + 1.5
...
result = prob.solve(verbose=True, solver = test_solver, x0 = b_upscale, eps_abs = eps_abs_rel, eps_rel=eps_abs_rel,max_iters=max_iters) # solve problem
x = x.value
