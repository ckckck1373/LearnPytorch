...
def get_kernel(mv_x, mv_y):
    ...
    line = np.arange(-gaussian_N//2.0, gaussian_N//2.0 + 1)
    x, y = np.meshgrid(line, line)
    D_x = x + (upsample_factor-1.0)/2.0 - mv_x * upsample_factor
    D_y = y + (upsample_factor-1.0)/2.0 - mv_y * upsample_factor
    
    gau_kernel = np.exp(-(D_x**2 + D_y**2) / (2.0 * gaussian_std**2))
    gau_kernel = gau_kernel / np.sum(gau_kernel)
    
    gau_rgb = np.dstack([gau_kernel]*3)
...
#prob = Problem(norm1( subsample(conv(gau_rgb,x, dims=2),(4,4,1)) - b ) + lamb * group_norm1(grad(x, dims = 2), [3])) 
# formulate problem
E1 = norm1( subsample(conv(gau_rgb1,x, dims=2),(4,4,1)) - np_img1 )
E2 = norm1( subsample(conv(gau_rgb2,x, dims=2),(4,4,1)) - np_img2 )
E3 = norm1( subsample(conv(gau_rgb3,x, dims=2),(4,4,1)) - np_img3 )
E4 = norm1( subsample(conv(gau_rgb4,x, dims=2),(4,4,1)) - np_img4 )

prob = Problem(E1 + E2 + E3 + E4 + lamb * group_norm1(grad(x, dims = 2), [3]))
#=====================================================

result = prob.solve(verbose=True, solver = test_solver, x0 = b1_upscale, eps_abs = eps_abs_rel, eps_rel=eps_abs_rel,max_iters=max_iters) # solve problem
x = x.value
